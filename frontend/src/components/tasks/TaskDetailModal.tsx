...
import { FiCalendar, FiFlag, FiPaperclip, FiTag, FiSend, FiDownload, FiEdit, FiTrash2 } from 'react-icons/fi';
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription>
            {task
              ? 'Detailed information about this task, including status, priority, assignee, and activity.'
              : 'Task details will appear here when a task is selected.'}
          </DialogDescription>
        </DialogHeader>
        ...
        {task ? (
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <span className="text-sm">{formatDistanceToNow(task.createdAt, { addSuffix: true })}</span>
                  </div>

                  <div>
                    <h4 className="font-medium text-sm text-muted-foreground mb-2">Updated</h4>
                    <span className="text-sm">{formatDistanceToNow(task.updatedAt, { addSuffix: true })}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="py-12 text-center text-muted-foreground">No task selected.</div>
        )}
      </DialogContent>
    </Dialog>
  );
